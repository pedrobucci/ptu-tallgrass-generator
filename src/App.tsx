...
import { appFileUrl, cacheTerrainBackground, clearImageFiles, exportSettings, exportTerrain, musicUrl, removeMusic, resolveEncounterImage, storeMusic } from "./media";
import { importTerrain, previewTerrain, terrainAsFile, type ImportPreview } from "./terrain-service";
import { DEFAULT_SETTINGS, encounterFrequencies, rarities, type AppSettingsV1, type EncounterResult, type Rarity, type Terrain, type TerrainFileV1 } from "./types";
...
export default function App() {
  ...
  const [editingTerrain, setEditingTerrain] = useState<Terrain | null | undefined>(undefined);
  ...
  async function navigate(nextPage: Page) {
    if (editorDirty && !await askConfirmation("Descartar as alterações deste terreno?", { title: "Alterações não salvas", kind: "warning" })) return;
    setEditorDirty(false); setEditingTerrain(undefined); setPage(nextPage);
  }
  ...
  async function saveEditedTerrain(file: TerrainFileV1) {
    const id = await importTerrain(file, editingTerrain?.id ?? null);
    await refresh();
    await selectTerrain(id);
    setEditingTerrain(undefined);
    setEditorDirty(false);
    setNotice(editingTerrain ? "Terreno atualizado com sucesso." : "Terreno criado com sucesso.");
    if (file.background_image_url) void cacheTerrainBackground(id, file.background_image_url).then(async (path) => { if (path) { await setTerrainBackgroundPath(id, path); await refresh(); } });
  }
  ...
  return <div className="app-shell" style={backgroundUrl ? { "--terrain-background": `url(${backgroundUrl})` } as React.CSSProperties : undefined}>
    ...
    <aside className="sidebar"><div className="brand"><div className="brand-mark">╱╲</div><div><strong>PTU</strong><span>Encounter Generator</span></div></div><nav aria-label="Navegação principal">{navItems.map((item) => <button key={item.id} className={page === item.id ? "active" : ""} onClick={() => void navigate(item.id)}><span aria-hidden>{item.glyph}</span>{item.label}</button>)}</nav><p className="catalog-version">Catálogo {pokemonCatalogSnapshot.catalog_version}<br />{pokemonCatalogSnapshot.entries.length} espécies</p></aside>
    <main className="content">
      ...
      {page === "terrains" && <><header className="page-header actions-header"><div><p className="eyebrow">Biblioteca local</p><h1>Terrenos</h1><p>Crie, edite, importe e exporte as tabelas usadas nos encontros.</p></div><div className="inline-actions"><button className="button secondary" onClick={() => { setEditingTerrain(null); setEditorDirty(false); }}>Novo terreno</button><button className="button primary" onClick={() => void openImport()}>Importar JSON</button></div></header>{editingTerrain !== undefined ? <TerrainEditor terrain={editingTerrain} terrains={terrains} onSave={saveEditedTerrain} onCancel={() => setEditingTerrain(undefined)} onDirtyChange={setEditorDirty} /> : !terrains.length ? <section className="panel empty-card"><div className="grass-mark">╱╲╱╲</div><h2>Nenhum terreno criado</h2><p>Crie um terreno pela interface ou importe um JSON.</p><div className="inline-actions centered-actions"><button className="button secondary" onClick={() => setEditingTerrain(null)}>Criar terreno</button><button className="button primary" onClick={() => void openImport()}>Escolher arquivo</button></div></section> : <div className="terrain-layout"><section className="terrain-list">{terrains.map((terrain) => <button key={terrain.id} className={selectedTerrain?.id === terrain.id ? "selected" : ""} onClick={() => void selectTerrain(terrain.id)}><strong>{terrain.name}</strong><span>{frequencyLabels[terrain.encounter_frequency]} · {terrain.pokemon_table.length} entradas</span></button>)}</section>{selectedTerrain && <section className="panel terrain-detail"><div className="section-heading"><div><p className="eyebrow">Schema {selectedTerrain.schema_version}</p><h2>{selectedTerrain.name}</h2></div><div className="inline-actions"><button className="button secondary" onClick={() => { setEditingTerrain(selectedTerrain); setEditorDirty(false); }}>Editar</button><button className="button ghost" onClick={() => void exportTerrain(selectedTerrain).then((saved) => saved && setNotice("Terreno exportado."))}>Exportar JSON</button><button className="button ghost" onClick={() => validateStored(selectedTerrain)}>Validar</button><button className="button danger" onClick={() => void removeTerrain(selectedTerrain.id)}>Excluir</button></div></div><dl className="terrain-summary"><div><dt>Níveis</dt><dd>{selectedTerrain.min_lvl ?? 1}–{selectedTerrain.max_lvl ?? 100}</dd></div><div><dt>Frequência</dt><dd>{frequencyLabels[selectedTerrain.encounter_frequency]}</dd></div><div><dt>Shiny</dt><dd>{selectedTerrain.shiny_rate == null ? "Global" : selectedTerrain.shiny_rate === 0 ? "Nunca" : `1 em ${Math.round(1 / selectedTerrain.shiny_rate).toLocaleString("pt-BR")}`}</dd></div></dl><div className="pokemon-table"><div className="table-head"><span>Pokémon</span><span>Raridade</span><span>Nível</span></div>{selectedTerrain.pokemon_table.map((entry) => { const pokemon = pokemonCatalog.get(entry.national_dex); return <div key={entry.id}><span><b>#{String(entry.national_dex).padStart(4, "0")}</b> {pokemon?.display_name ?? "Desconhecido"}</span><span>{rarityLabels[entry.rarity]}</span><span>{entry.min_lvl ?? selectedTerrain.min_lvl ?? 1}–{entry.max_lvl ?? selectedTerrain.max_lvl ?? 100}</span></div>; })}</div></section>}</div>}</>}
    ...
    <nav className="mobile-nav" aria-label="Navegação principal">{navItems.map((item) => <button key={item.id} className={page === item.id ? "active" : ""} onClick={() => void navigate(item.id)}><span>{item.glyph}</span>{item.label}</button>)}</nav>
